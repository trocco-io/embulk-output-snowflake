    ...
    if (searchString != null && escapeString != null) {
      ...
      searchString =
          searchString.replaceAll(
              Pattern.quote(escapeString), Matcher.quoteReplacement(escapeString + escapeString));
      for (String specialChar : SEARCH_STRING_SPECIAL_CHARS) {
        if (specialChar.equals(escapeString)) {
          throw new IllegalArgumentException(
              "Special char " + specialChar + " cannot be an escape char");
        }
        searchString =
            searchString.replaceAll(
                Pattern.quote(specialChar), Matcher.quoteReplacement(escapeString + specialChar));
      }
    }
    return searchString;
  }

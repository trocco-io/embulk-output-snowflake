...
public class ColumnSetterFactory {
  private static final Logger logger = LoggerFactory.getLogger(ColumnSetterFactory.class);
  ...
  public ColumnSetter newColumnSetter(JdbcColumn column, JdbcColumnOption option) {
    switch (option.getValueType()) {
      ...
      case "string":
        return new StringColumnSetter(
            batch, column, newDefaultValueSetter(column, option), newTimestampFormatter(option));
      case "nstring":
        return new NStringColumnSetter(
            batch, column, newDefaultValueSetter(column, option), newTimestampFormatter(option));
      case "date":
        return new SqlDateColumnSetter(
            batch, column, newDefaultValueSetter(column, option), newCalendar(option));
      case "time":
        return new SqlTimeColumnSetter(
            batch, column, newDefaultValueSetter(column, option), newCalendar(option));
      case "timestamp":
        return new SqlTimestampColumnSetter(
            batch, column, newDefaultValueSetter(column, option), newCalendar(option));
      case "decimal":
      ...
      default:
        throw new ConfigException(
            String.format(
                "Unknown value_type '%s' for column '%s'",
                option.getValueType(), column.getName()));
    }
  }
  ...
  public ColumnSetter newCoalesceColumnSetter(JdbcColumn column, JdbcColumnOption option) {
    logger.info("Coerce type {}", column.getSqlType());
  ...
  private static UnsupportedOperationException unsupportedOperationException(JdbcColumn column) {
    throw new UnsupportedOperationException(
        String.format(
            "Unsupported type %s (sqlType=%d, size=%d, scale=%d)",
